...
ggplot(data = edx, aes(rating)) + geom_histogram(binwidth = 0.5)
...
# Lastly, when we compute the average rating for each week and plot this average against day, we see the existance of a time effect.
edx$timestamp
...
# Now we implemented the time effect to the model



model_3_rmse <- RMSE(predicted_ratings, test_set$rating)

rmse_results <- bind_rows(rmse_results, data_frame(method="Movie + User + Time Effects Model",  
                                                   RMSE = model_3_rmse ))

rmse_results %>% knitr::kable()

# Can we make it even better? Yes we can.
